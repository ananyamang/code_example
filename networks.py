...
def update_weights(config, network, optimizer, batch, train_results):
    ...
    def loss():
        ...
        val, reward, policy_logits, hidden_representation = network.initial_inference(state_batch)
        ...
        cem_value_loss = tf.nn.softmax_cross_entropy_with_logits(val, target_value_batch)
        cem_policy_loss = tf.nn.softmax_cross_entropy_with_logits(target_policy_batch, policy_logits)
        total_value_loss += cem_value_loss * 0.25
        total_policy_loss += cem_policy_loss
        
        loss = 0.25 * cem_value_loss + cem_policy_loss
        ...
        for actions_batch, targets_batch in zip(actions_batch, targets_recurrent_batch):
            ...
            one_hot_A = tf.one_hot(actions_batch, depth=actions_batch.size)
            # YOUR CODE HERE:
            # Create conditioned_representation: concatenate representations with actions batch
            # Recurrent step from conditioned representation: recurrent + prediction networks
            conditioned_representation = tf.concat(hidden_representation, one_hot_A)
            val, reward, policy_logits, hidden_representation = network.recurrent_inference(conditioned_representation)
            ...
            # Remember to scale value loss!
            # Add to total losses
            cem_value_loss = tf.nn.softmax_cross_entropy_with_logits(target_value_batch, val)
            cem_policy_loss = tf.nn.softmax_cross_entropy_with_logits(target_policy_batch, policy_logits)
            rewards_loss = tf.losses.mean_squared_error(reward, target_reward_batch)

            loss_step = 0.25 * cem_value_loss + cem_policy_loss + rewards_loss
            loss_step = loss_step * (1/config.num_unenroll_steps)
            loss += loss_step
            # do i have to add the losses to the policy. reward, val totals
            ...
            total_value_loss += cem_value_loss * 0.25
            total_policy_loss += cem_policy_loss
            total_reward_loss += rewards_loss
